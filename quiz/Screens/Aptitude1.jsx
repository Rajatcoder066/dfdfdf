...
import {  Apitiude } from '../src/Arithmatic1';
...
const HomeScreen = (props) => {
  ...
  const [Aptitude1, setQuestions] = useState( Apitiude);
  const listRef = useRef();
  const [modalVisible, setModalVisible] = useState(false);
  const OnSelectOption = (index, x) => {
    const tempData = Aptitude1;
        ...
        {currentIndex ==   Apitiude.length ? (
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              if (Aptitude1[currentIndex - 1].marked !== -1) {
                if (currentIndex < Aptitude1.length) {
                  listRef.current.scrollToIndex({
                    animated: true,
                    index: currentIndex,
                  });
