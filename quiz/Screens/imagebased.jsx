...
import { View, Text, Button, DrawerLayoutAndroid, StyleSheet, BackHandler,Alert,Dimensions,FlatList,TouchableOpacity,Modal,Image,ImageBackground
} from 'react-native';
import { useNavigation,StackActions,useFocusEffect } from '@react-navigation/native';
import QuestionItem1 from '../QuestionItem1';
import { imagebased } from '../src/imagebased';
...
const HomeScreen = (props) => {
  ...
  const [questions, setQuestions] = useState(imagebased);
    ...
    tempData.map((item, ind) => {
      if (index == ind) {
        if (item.marked !== -1) {
          ...
          item.marked = x;
         console.log(item.correct,item.marked,item.imagess);
   ...
   source={require('../assets/rain.jpg')} // Provide the path to your image
    style={styles.image}>
      ...
      <View
        ...
        <Text
          style={{
           ...
           Image based :{' ' + currentIndex + '/' + imagebased.length}
        </Text>
        <Text
          style={{
            marginRight: 20,
      ...
      <View
        ...
        {currentIndex == imagebased.length ? (
          <TouchableOpacity
            style={{
